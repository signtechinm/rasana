...
export async function getCollection(collection: "brands" | "catalogues" | "news"): Promise<ContentItem[]> {
  ...
  try { const query = collection === "brands" ? "?where[status][equals]=published&sort=displayOrder&limit=100" : "?limit=100"; const response = await fetch(`${endpoint}/api/${collection}${query}`, { next: { revalidate: 60 } }); if (!response.ok) return fallback; return (await response.json()).docs as ContentItem[]; } catch { return fallback; }
}
